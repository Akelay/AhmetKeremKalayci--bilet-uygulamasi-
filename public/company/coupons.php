...
// Varsayılan firma
$r = $pdo->query("SELECT id FROM Bus_Company ORDER BY name LIMIT 1")->fetch(PDO::FETCH_ASSOC);
$defaultCompanyId = $r['id'] ?? null;
...
if ($op === 'create') {
    ...
    $stmt = $pdo->prepare("INSERT INTO Coupons(id, code, discount, usage_limit, company_id, expire_date)
                           VALUES(:id, :code, :discount, :usage_limit, :company_id, :expire_date)");
    try {
        $stmt->execute([
            ':id'          => uuid(),
            ':code'        => $code,
...
if ($defaultCompanyId) {
    ...
    $st->execute([':cid' => $defaultCompanyId]);
    $coupons = $st->fetchAll(PDO::FETCH_ASSOC);
} else {
    // Firma ID yoksa, sadece global kuponları göster
    $st = $pdo->prepare("SELECT id, code, discount, usage_limit, expire_date, created_at
                         FROM Coupons
...
page_header('Kuponlar');
$flash = get_flash();
if ($flash) echo "<p class='flash {$flash['type']}'>".e($flash['msg'])."</p>";
?>

<h2>Kupon Oluştur</h2>
<form method="post" action="/company/coupons.php">
    ...
    <?php else: ?>
      <?php foreach ($coupons as $c): ?>
        <tr>
          ...
          <td>
            <a href="/company/coupons.php?edit=<?= urlencode($c['id']) ?>"
            ...
            <form method="post" action="/company/coupons.php" style="display:inline" onsubmit="return confirm('Silinsin mi?');">
        ...
        <?php if ($editId && $editId === $c['id']): ?>
          <tr>
            <td colspan="7" style="background:#fafafa;">
              <form method="post" action="/company/coupons.php" style="padding:10px; border:1px solid #ddd;">
                ...
                <a href="/company/coupons.php">Vazgeç</a>
              </form>
            </td>
          </tr>
        <?php endif; ?>
